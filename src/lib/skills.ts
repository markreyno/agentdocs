...
export function stripEmbeddedDocumentFromTemplate(template: string): string {
  ...
  return `${withoutDoc}\n\nDo not assume the full manuscript is in this message. Use search_passages for conceptual prose discovery; use doc_status, search_outline, search_sentences, get_node, or get_story_blocks for exact navigation and edits. Do not re-fetch unchanged loaded nodes.`
}
